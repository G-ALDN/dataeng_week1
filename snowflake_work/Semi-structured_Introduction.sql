...
-- This will load our data so that each top level object in json is treated like a new row
-- lets truncate and run the above query again after:
-- STRIP_OUTER_ARRAY = TRUE in file format
-- AND copy data back in, forcing while we do so due to load history
-- COPY INTO FILMS_ELT
-- FROM @FILMS_STAGE/films.json
-- FILE_FORMAT = JSON_FILE_FORMAT
-- FORCE=TRUE;
...
-- Title with capital T will return null
SELECT 
json_variant:id as id, 
json_variant:Title as title, 
...
-- Flatten table function example
-- Here we see the shape of ratings before we flatten it
...
-- $1 represents all the data from the json object
